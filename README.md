...
- AI 答疑页支持上传或拍照题目图片，浏览器本地 OCR 识别后自动填入题目框。
...
- 电脑端：点击“上传题目图片”，选择本地题目照片或截图。
- 手机和平板：点击同一个按钮，可拍照或从相册选择。
- 系统会在浏览器本地识别图片文字，并自动填入题目输入框。
- 识别后建议家长或孩子简单检查一遍文字，再点击“开始答疑”。

这个功能不调用云端 AI API，不产生 AI API 调用费用。OCR 识别资源已经放在项目的 `public/ocr` 目录中，首次识别可能会花几秒钟加载资源；清晰印刷题效果更好，手写字、复杂公式和几何图仍可能需要手动修改。
